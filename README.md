...
ピンアサインは `leader/pins.h` を参照し、配線に合わせて定数を調整してください。既定では A1〜A3 を前方中央センサ、A4/A5 をモードに応じた外側・後方センサ、A0 をモード選択用可変抵抗、3〜8 をモータ制御、9〜13 をセンサ表示 LED、2 を警告 LED に割り当てています。
...
| モード | 想定競技 | 動作概要 |
| :-- | :-- | :-- |
| `RUNMODE_RECIP` | 直線コース往復1 | 往路前進 → 復路は向きを変えずに後退。後退時は後方 2 センサを用いる |
| `RUNMODE_UTURN` | 直線コース往復2 | 往路前進 → 端点で 180° 回頭 → 復路前進。後退フェーズは存在せず、端点検出までは後方センサを利用 |
| `RUNMODE_LOOP` | 周回コース | 端点で停止せず、常に前進しながら前方 5 センサでライン追従 |

選択方法は 2 通りあります。

- **コンパイル時既定値**: `leader/run_mode.h` の `COMPILE_TIME_RUNMODE` を書き換えてビルドします。
- **可変抵抗入力**: `leader/run_mode_pot.ino` のロジックで A0 (`RUNMODE_POT_PIN`) に接続した可変抵抗を読み取り、起動時に 3 分割の電圧域でモードを切り替えます。利用しない場合は `RUNMODE_POT_ENABLED` を `false` に設定してください。

### センサモードと配線

`leader/sensors.ino` は走行モードに応じてセンサ配置を切り替える `SensorMode` を持ちます。

- `SensorMode::Front3Rear2`（`RUNMODE_RECIP` / `RUNMODE_UTURN`）: A1〜A3 を前方左・中央・右センサ、A4 を後方左、A5 を後方右として使用します。後退中のライン追従や端点検出に後方センサを活用します。
- `SensorMode::Front5`（`RUNMODE_LOOP`）: A1〜A5 の 5 本すべてを前方センサとして扱い、A4 を前方最左、A5 を前方最右センサに割り当てます。常に前進しながら広い視野でラインを追従します。

周回走行モードを使用する場合は A4/A5 を前方最左・最右センサに、直線走行（後退・U ターン）モードでは A4/A5 を後方左・右センサに接続するよう配線してください。モードを切り替える際は必要に応じて配線変更またはコネクタの差し替えを行います。
...
- **センサ判定**: `THRESHOLD`（黒白境界）、`HYST`（ヒステリシス）、`LINE_WHITE` / `LINE_BLACK`（目標とする反射値）、`LINE_EPS`（全白判定の閾値）。
- **基準速度**: `BASE_FWD` / `BASE_BACK`（前進・後退の基準 PWM）、`BASE_FWD_MIN` / `BASE_BACK_MIN`（減速時の下限）、`BASE_SPEED_ALPHA_ACCEL` / `BASE_SPEED_ALPHA_DECEL`（加減速レスポンス）。
- **PID ゲイン**: `KP_*` / `KI_*` / `KD_*`（前進・後退で個別に設定）、`PID_I_LIMIT`（I 項のアンチワインドアップ）。
- **カーブ減速**: `CURVE_E_GAIN` / `CURVE_E_EXP`（誤差に応じた減速）、`CURVE_D_GAIN`（変化量による減速）、`CORR_EXP`（ハンドル補正の非線形指数）。
